...
│   ├── bme680_reader.py   # Real BME680 sensor reader (I2C, 0x77)
...
**IoT #1 — Real BME680 sensor (Feb 18, 2026)**

| Sensor | Model | I2C Address | Measures | Status |
|--------|-------|-------------|----------|--------|
| **Environment** | Waveshare IU-BME680 | **0x77** (I2C_ADDR_SECONDARY) | Temp, Humidity, Pressure, Gas/VOC | Real hardware |

`SIMULATION_MODE = False` on IoT #1 only. All other devices remain simulated.

**BME680 I2C Wiring (IoT #1 only):**
...
Disconnected wires (not needed for I2C): Green (CS), Orange (ADDR/MISO).

Pin 9 (closer GND) was unavailable (used by SN74 buffer OE1), so Pin 25 is used instead.

**Software:** `sensors/bme680_reader.py` — drop-in replacement for `SimulatedSensors`. Same `read_all(current_pwm)` interface. `telemetry/collector.py` auto-selects BME680Sensors when `SIMULATION_MODE = False`, with fallback to simulator if init fails.

**BME680 sensor notes:**
- Gas resistance (ohms) is converted to approximate VOC ppb using a rolling baseline (75th percentile of last 300 readings)
- CO2 and PM2.5 are estimated from VOC (BME680 does not measure these directly)
- Gas heater needs ~30 seconds to stabilize after power-on (`heat_stable` flag in `_sensor_state`)
- Default I2C address is 0x77; can be changed to 0x76 by shorting ADDR pad on the board
- Library: `bme680` (pip, installed with `--break-system-packages` on Pi OS Bookworm)

**IoT #2, #3, #4 — Simulated sensors**

| Sensor | Model | Purpose | Status |
|--------|-------|---------|--------|
| Pressure | SDP810-500Pa | Differential pressure | Simulated |
| VOC | SGP30 | Volatile Organic Compounds | Simulated |
| Power | INA219 | Power consumption | Simulated |
| Temp/Humidity | BME280 | Temperature & humidity | Simulated |

`SIMULATION_MODE = True` — all readings are generated by `sensors/simulator.py`.
